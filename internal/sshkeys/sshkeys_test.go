...
// ---------------------------------------------------------------------------
// Integration: DistributePublicKey + RemovePublicKey on localhost
// ---------------------------------------------------------------------------

// TestDistributeAndRemovePublicKey_Localhost is an integration test that
// exercises the full distribute → remove cycle on localhost.
//
// It is skipped unless ssh to localhost with BatchMode is available.
func TestDistributeAndRemovePublicKey_Localhost(t *testing.T) {
	...
	if err := DistributePublicKey(ctx, host, kp.PublicKey); err != nil {
		t.Fatalf("DistributePublicKey: %v", err)
	}

	// Step 2: verify the comment appears in authorized_keys.
	checkPresent := exec.CommandContext(ctx, "ssh",
		"-o", "BatchMode=yes",
		"-o", "StrictHostKeyChecking=no",
		"-o", "ConnectTimeout=5",
		"localhost", "--",
		fmt.Sprintf("grep -q %s ~/.ssh/authorized_keys", shellescape(kp.Comment)),
	)
	if err := checkPresent.Run(); err != nil {
		t.Errorf("expected key comment %q in authorized_keys after distribute: %v", kp.Comment, err)
	}
	...
	// Step 4: verify the comment is no longer present.
	var grepOut strings.Builder
	checkGone := exec.CommandContext(ctx, "ssh",
		"-o", "BatchMode=yes",
		"-o", "StrictHostKeyChecking=no",
		"-o", "ConnectTimeout=5",
		"localhost", "--",
		fmt.Sprintf("grep -c %s ~/.ssh/authorized_keys || true", shellescape(kp.Comment)),
	)
	checkGone.Stdout = &grepOut
	_ = checkGone.Run()
	count := strings.TrimSpace(grepOut.String())
	if count != "0" && count != "" {
		t.Errorf("key comment %q should be gone from authorized_keys; grep -c returned %q", kp.Comment, count)
	}
}
